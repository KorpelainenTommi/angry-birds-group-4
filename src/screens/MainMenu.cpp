...
MainMenu::MainMenu(Application& app): Screen(app){
    ...
    auto level1 = std::make_shared<Button>(0 VH, 1 VW, 50 VH, 72 VW, [](){std::cout << "level1" << std::endl;});
    level1->SetText("level1");
    list->InsertElement(level1);
    menu_.push_back(level1);

    auto level2 = std::make_shared<Button>(0 VH, 1 VW, 50 VH, 72 VW, [](){std::cout << "level2" << std::endl;});
    level2->SetText("level2");
    list->InsertElement(level2);
    menu_.push_back(level2);

    auto level3 = std::make_shared<Button>(0 VH, 1 VW, 50 VH, 72 VW, [](){std::cout << "level3" << std::endl;});
